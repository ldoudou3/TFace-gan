...
class TrainTask(BaseTask):
    ...
    def loop_step(self, epoch):
        ...
        for step, samples in enumerate(self.train_loader):
            ...
            torch.distributed.barrier()  # 确保所有进程都完成数据加载
            ...
            if self.amp:
                ...
                self.scaler.unscale_(discriminator_opt)  # D
                torch.nn.utils.clip_grad_norm_(self.backbone.parameters(), max_norm=5.0)
                torch.nn.utils.clip_grad_norm_(self.backbone_discriminator.parameters(), max_norm=5.0)  # D
...
if __name__ == '__main__':
    ...
    wandb.init(
        project=timestamp+"planB",  # 替换为你的项目名称
